...
void createCtoLnext(vector<vector<string> >C,vector<vector<string> > &L,int k,map<string,vector<string> >records,map<vector<string> ,int> l1map,map<vector<string> ,int> &Lmap,int minSupp)
{
    int csize = C.size() ;
    int x = k-1 ;
    ...
    */
    ...
    while(1)
    {
       ...
       // cout<<"-------------------------------------------------------"<<endl;
        ...
        createCtoLnext(C,L,k,records,l1map,Lmap,minSupp) ;
